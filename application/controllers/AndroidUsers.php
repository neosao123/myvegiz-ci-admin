...
class AndroidUsers extends CI_Controller
{
    ...
    public function getAndroidUsersList()
    {
        ...
        $emailId = $this->input->post("emailId") ?? $this->input->get("emailId");
        $status = $this->input->post("isActive") ?? $this->input->get("isActive");
        $cityCode = $this->input->post("cityCode") ?? $this->input->get("cityCode");
        $export = $this->input->post('export') ?? $this->input->get('export');
    ...
    {
        $code = $this->input->post('code') ?? $this->input->get('code') ?? $this->uri->segment(3);
        //Activity Track Starts
        $addID = $this->session->userdata['logged_in' . $this->session_key]['code'];
        $userRole = $this->session->userdata['logged_in' . $this->session_key]['role'];
        $userName = $this->session->userdata['logged_in' . $this->session_key]['username'];
        ...
        if ($data['query']->num_rows() > 0) {
            ...
            if ($citycode) {
                ...
                $data['cityname'] = $city->num_rows() > 0 ? $city->result()[0]->cityName : "";
            }
            else {
                $data['cityname'] = "";
            }
        ...
        if ($this->input->is_ajax_request()) {
            $this->load->view('dashboard/androidUsers/view_modal', $data);
        }
        else {
            $this->load->view('dashboard/header');
