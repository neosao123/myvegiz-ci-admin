...
class DeliveryApi extends REST_Controller {
	...
	public function dashboard_post()
	{
		...
		if ($postData["code"] != '')
		{
			...
			$cond=array("ordermaster.isActive"=>1,"ordermaster.orderStatus"=>"DEL","ordermaster.editID"=>$postData["code"],"ordermaster.editDate >="=>date('Y/m/d',strtotime("-2 days")));
			$orderBy = array('ordermaster' . ".id" => 'ASC');
			$join = array();
			$joinType=array();
			$like=array();
			...
			if($areaCodes!=""){
				...
				$cond=array("ordermaster.isActive"=>1,"ordermaster.orderStatus"=>"RJT","ordermaster.editID"=>$postData["code"],"ordermaster.editDate >="=>date('Y/m/d',strtotime("-2 days")));
				$orderBy = array('ordermaster' . ".id" => 'ASC');
				$join = array();
				$joinType=array();
				$like=array();
	...
	public function getPendingOrders_post()
	{
		...
		if($postData['code'] !='' && $postData['offset']!="")
		{
			...
			if($orderStatus==""){
				...
				$m_condition= " AND ordermaster.orderStatus = 'PLC' AND ordermaster.editId='".$postData['code']."'";
			}
			else
			{
				$m_condition="";
	...
	public function getDeliveredOrders_post()
	{
		...
		if($postData['code'] !='' && $postData['offset']!="")
		{
			...
			if($areaCodes !='')
			{
					...
					$join = array('clientmaster'=>'clientmaster.code = ordermaster.clientCode','orderstatusmaster' => 'ordermaster' . '.orderStatus=' . 'orderstatusmaster' .'.statusSName','paymentstatusmaster' => 'ordermaster' . '.paymentStatus=' . 'paymentstatusmaster' .'.statusSName');
					$joinType=array('clientmaster' =>'left','orderstatusmaster' =>'inner','paymentstatusmaster' =>'inner');
					$cond=array("ordermaster.orderStatus"=>'DEL',"ordermaster.editID"=>$postData['code'],"ordermaster.editDate >="=>date('Y/m/d',strtotime("-2 days")));
	...
	public function getRejectedOrders_post()
	{
		...
		if($postData['code'] !='' && $postData['offset']!="")
		{
			...
			$cond=array("ordermaster.orderStatus"=>"RJT","ordermaster.editID"=>$postData["code"],"ordermaster.editDate >="=>date('Y/m/d',strtotime("-2 days")));
			$orderBy = array('ordermaster' . ".id" => 'DESC');
			$like=array();
			$limit="10";
			$offset=$postData['offset'];
