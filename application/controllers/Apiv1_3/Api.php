...
class Api extends REST_Controller {
	...
	public function userProfile_post()
	{
		...
		{ 
			...
			// if($resultData[0]['latitude']==null){
				// $resultData[0]['latitude']="16.704987";
				// $resultData[0]['longitude']="74.243256";
			// }
	...
	public function placeOrder_post()
	{
		...
		if ($postData["clientCode"] != '' && $postData["paymentMode"] !='' && $postData["areaCode"] !='')
		{
			$clientCode = $postData["clientCode"];
	...
	function getOffers_get()
	{
		$tableName="offers";
		$todayDate=date('Y-m-d');;
		$orderColumns = array("offers.*" );
		$cond=array('offers' . ".startDate <=" =>$todayDate,'offers' . ".expireDate >=" =>$todayDate,'offers' . ".isActive" =>1);
		...
		{
			return $this->response(array("status" => "400","totalRecords"=>0,"message"=>"no records found"), 200);
		}
	}
