...
class DeliveryApi extends REST_Controller
{
	...
	public function getOrdersByStatus_post()
	{
		...
		if ($postData['code'] != '' && $postData['orderStatus'] != "") {
			...
			$cond = array("ordermaster.deliveryBoyCode" => $postData['code']);
			$orderBy = array('ordermaster' . ".id" => 'ASC');
			$like = array();
			$offset = "";
			$groupByColumn = array();
			...
			$cond = array("vendorordermaster.deliveryBoyCode" => $postData['code']); 
			...
			if ($resultQuery) { 
				...
				for ($i = 0; $i < sizeof($clientOrderList); $i++) {
					...
					if ($vendorData) {
						...
						$vd['vendorContact'] = $vendor['entityContact'];
			...
			if(!empty($clientOrderLista )){
				...
				$this->response(array("status" => "200", "message" => " Order details", "result" => $finalResult,"re"=>$r), 200);
			} else { 
				$this->response(array("status" => "300", "message" => "No Data Found","re"=>$r), 200);
			} 
		} else {
			$this->response(array("status" => "400", "message" => " * are Required field(s)."), 400);
		}
	...
	public function updateOrderStatus_post()
	{
		...
		if ($postData['code'] != "" && $postData['orderStatus'] != "" && $postData['orderCode'] != "") {
			...
			if ($orderStatus == "PLC") {
				...
				$reason ="Order is delivered successfully";
			...
			if ($bookLineResult != 'false') {			
				...
				if($res) {
					...
					if ($res) {
						...
						$dataupCnt['deliveryBoyCode'] = $postData['code'];
						...
						if($orderStatus == "PUP") {
							...
							if($grandTotal>250){  
								$percnVal = round($grandTotal * (10/100));
								$delCom = $percnVal; 
							} else {
								$delCom = 25;
							}
		...
		if($postData['code']!=""){
		  ...
		  $code = $postData['code'];
			...
			if($resultData){
				...
				return $this->response(array("status" => "200","deliveryAmountEarned"=>$totalAmount, "msg" => "Data found","result"=>$res), 200);
			} else{
				return $this->response(array("status" => "300", "msg" => "No Data Found"), 200);
			}
		} else {
