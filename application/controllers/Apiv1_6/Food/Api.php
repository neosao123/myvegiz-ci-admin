...
class Api extends REST_Controller
{
			...
			if($serviceable==1){
				...
				if ($insertResult) {
					...
					if (!empty($cart)) {
						...
						if ((isset($postData["coupanCode"])) && $postData['coupanCode'] != "") {
	...
	public function getVendorOrderList_post()
	{
		...
		if ($postData["clientCode"] != '') {
			...
			$cond = array('vendorordermaster' . ".clientCode" => $clientCode);
...
// 	}
	...
	public function getCoupanDetails_post()
	{
		...
		if ($postData['clientCode'] != "" && $postData["coupanCode"] != "" && $postData['vendorCode'] != "" && $postData['cartAmount'] != "") {
			...
			$condition = array('vendoroffer.coupanCode' => $couponCode, 'vendoroffer.vendorCode' => $vendorCode, 'vendoroffer.isActive' => 1, "vendoroffer.perUserLimit>" => 1);
			$extraCondition = "( '" . $today . "' between vendoroffer.startDate and vendoroffer.endDate)";
			...
			if ($Result) {
				...
				if ($minimumAmount <= $cartAmount) {
					//check used or not 
					$condition1 = array(
						'couponusesdetail.vendorOfferCode' => $vendorCode,
					...
					if ($clientUseCouponResult) {
						...
						if ($userLimit < $perUserLimit) {
							...
							return $this->response(array("status" => "200", "nextLimit" => $nextLimit, "message" => "Coupon Applicable", "result" => $data), 200);
						} else {
							return $this->response(array("status" => "300", "message" => "Coupon Already Used"), 200);
						}
					} else {
						//not single use return coupon
						return $this->response(array("status" => "200", "nextLimit" => 1, "message" => "Coupon Applicable", "result" => $data), 200);
					}
				} else {
