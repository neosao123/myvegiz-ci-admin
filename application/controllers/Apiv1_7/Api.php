...
class Api extends REST_Controller
{
	...
	public function registration_post()
	{
		...
		if ($postData["name"] != '' && $postData["mobile"] != '' && $postData["password"] != '' && $postData["cityCode"] != "") {
			...
			$firebaseId=isset($postData['firebaseId'])?$postData['firebaseId']:"";
			...
			if ($insertResult != 'false') {
				if ((isset($postData['os_version'])) && ($postData['os_version'] != "")) {
					...
					$dataDevice['firebaseID'] = $firebaseId;
	...
	public function loginProcess_post()
	{
		...
		if ($postData["type"] != '' && $postData["userId"] != '' && $postData["userPassword"] != '') {
			...
			if ($postData["type"] == 'mobile') {
				...
				$clientData = $this->GlobalModel->selectDataByField('emailId', $email, 'clientmaster');
					...
					if ($p_result) {
						...
						for ($j = 0; $j < sizeof($product_result); $j++) {
							...
							if($clientCode!=""){
								...
								$cond2 = array('clientcarts' . ".productCode" => $product_result[$j]['code'], 'clientcarts' . ".clientCode" => $clientCode);
				...
				if ($resultQuery) {
					...
					for ($j = 0; $j < sizeof($limitProduct_result); $j++) {
							...
							if($clientCode!=""){
								$tableName2 = "clientcarts";
								$orderColumns2 = array("clientcarts.*");
								$cond2 = array('clientcarts' . ".productCode" => $limitProduct_result[$j]['code'], 'clientcarts' . ".clientCode" => $clientCode);
								$orderBy2 = array('clientcarts' . ".id" => 'DESC'); 
								$clientcarts = $this->GlobalModel->selectQuery($orderColumns2, $tableName2, $cond2, $orderBy2);
								if ($clientcarts) {
	...
	public function getCartList_post()
	{
		...
		if ($postData["clientCode"] != '' && $postData['cityCode'] != "") {
			...
			if ($res) {
				...
				$gst="0";
				return $this->response(array("status" => "200", "totalRecords" => sizeof($clientCartList), "minimumOrder" => $minOrder, "deliveryCharge" => $deliveryCharge,"gst"=>$gst, "result" => $data), 200);
			} else {
	...
	public function placeOrder_post()
	{
		...
		if ($postData["clientCode"] != '' && $postData["paymentMode"] != '' && $postData["areaCode"] != '' && $postData['cityCode'] != "") {
			...
			$couponCode = isset($postData["couponCode"])?$postData["couponCode"]:0;
			$subTotal =  isset($postData["subTotal"])?$postData["subTotal"]:0;
			$gst = isset($postData["gst"])?$postData["gst"]:0;
			$totalamount = $postData["totalPrice"];
			$vendorCode="myvegiz";
...
//reset password
	...
	public function maintenance_get()
	{
		...
		$maintenance_mode['messageTitle'] = "Update Your Application";
		$maintenance_mode['messageDescription'] = "Latest version is available...";
		return $this->response(array("status" => "200", "result" => $maintenance_mode), 200);
	}
	...
	public function getAppAlertMessage_post()
	{
		$postData = $this->post();
		if ($postData["clientCode"] != '') {
			$clientCode = $postData["clientCode"];
...
//vegee coupon list
	public function getVegeeCouponList_post(){
		$postData = $this->post();
		if($postData['clientCode']!=""){
			$today = date('Y-m-d');
			...
			$Result = $this->GlobalModel->selectQuery('vegitableoroffer.coupanCode as couponCode,vegitableoroffer.offerType,vegitableoroffer.discount,vegitableoroffer.minimumAmount,vegitableoroffer.perUserLimit,vegitableoroffer.termsAndConditions,vegitableoroffer.capLimit,vegitableoroffer.startDate,vegitableoroffer.endDate', 'vegitableoroffer', $condition, array(), array(), array(), array(), array(), "", "", $extraCondition);
			if ($Result) {
	...
	{
		$postData = $this->post();
		if ($postData['clientCode'] != "" && $postData["coupanCode"] != "" && $postData['cartAmount'] != "") {
