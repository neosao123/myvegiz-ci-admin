...
class Vendor extends REST_Controller
{
	...
	public function getDashboardCounts_post()
	{
		...
		if(isset($postData['restaurantCode']) && $postData['restaurantCode']!=""){
			...
			$pendingOrders_del = $placedOrders_del = $deliveredOrders_del =$rejectedOrders_del=$cancelledOrder_del = 0;
			$vendoroffer = $vendoritems = $dlbResetPasswordCounts = 0;
			
			$condition['vendorCode'] = $resCode;
			$condition['orderStatus'] = 'PND';
			$result = $this->GlobalModel->selectQuery("count(*) as Count","vendorordermaster",$condition);
			...
			$condition2['vendorCode'] = $resCode;
			$condition2['orderStatus'] = 'DEL';
			$result = $this->GlobalModel->selectQuery("count(*) as Count","vendorordermaster",$condition2);
			if($result) $deliveredOrders_del = $result->result_array()[0]['Count'];
			
			$condition2['vendorCode'] = $resCode;
			$condition2['orderStatus'] = 'RJT';
			$result = $this->GlobalModel->selectQuery("count(*) as Count","vendorordermaster",$condition2);
			if($result) $rejectedOrders_del = $result->result_array()[0]['Count'];
			
			$condition2['vendorCode'] = $resCode;
			$condition2['orderStatus'] = 'CAN';
			$result = $this->GlobalModel->selectQuery("count(*) as Count","vendorordermaster",$condition2);
			if($result) $cancelledOrder_del = $result->result_array()[0]['Count'];
