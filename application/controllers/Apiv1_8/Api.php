...
class Api extends REST_Controller
{
	...
	public function verifyRegisterOTPold_post()
	{
		$postData = $this->post();
		...
		if (isset($postData["offset"]) && $postData["offset"] != '' && isset($postData['cityCode']) && $postData['cityCode'] != "" && isset($postData['mainCategoryCode']) && $postData['mainCategoryCode'] != "") {
			...
			if ($category_result) {
				...
				for ($i = 0; $i < sizeof($category_result); $i++) {
					...
					$p_result = $this->GlobalModel->selectQuery($orderColumns, 'productmaster', $cond, $orderBy, $join, $joinType, $like, "5", "", $groupBy);
								...
								if ($isOnlinePayment) {
									...
									if ($users->num_rows() > 0) {
										...
										if (sizeof($firebaseIds) > 0) {
											$random = rand(0, 999);
											$datamsg = array("title" => 'Order ' . $insertResult, "message" => ' New Vegetable/Grocery order has been placed ', "order_id" => $insertResult, "random_id" => $random);
	...
	public function deleteUser_post()
	{
		...
		if (isset($getData['clientCode']) && $getData['clientCode'] != "") {
			...
			$extraCondition = "";
			$Result = $this->GlobalModel->selectQuery('clientmaster.*', 'clientmaster', $condition, array(), array(), array(), array(), "", "", array(), $extraCondition);
			...
			if ($Result) {
				...
				$resultData = $this->GlobalModel->doEdit($data, 'clientmaster', $getData['clientCode']);
				if ($resultData == true) {
					return $this->response(array("status" => "200", "message" => "User deleted successfully."), 200);
				}
				return $this->response(array("status" => "300", "msg" => "Something went to wrong."), 200);
