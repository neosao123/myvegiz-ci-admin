			...
			if ($orderVendor && $orderVendor->num_rows() > 0) {
				...
				if ($resultQuery != false) {
					...
					for ($i = 0; $i < $totalOrders; $i++) {
						...
						$linetableName = "vendororderlineentries";
						$lineorderColumns = array("vendororderlineentries.vendorItemCode,vendororderlineentries.quantity,vendororderlineentries.priceWithQuantity as priceWithQuantity,vendoritemmaster.itemName,vendoritemmaster.itemPhoto as itemImage");
						$linecond = array("vendororderlineentries.orderCode" => $clientOrderList[$i]['orderCode']);
						$lineorderBy = array('vendororderlineentries' . ".id" => 'ASC');
						$linejoin = array('vendoritemmaster' => 'vendororderlineentries.vendorItemCode=vendoritemmaster.code');
						$linejoinType = array('vendoritemmaster' => 'inner');
						...
						if ($orderProductRes) {
							$orderProductList = $orderProductRes->result_array();
							for ($j = 0; $j < sizeof($orderProductList); $j++) {
								$image = $this->GlobalModel->selectQuery('vendoritemmaster.itemPhoto', 'vendoritemmaster', array('vendoritemmaster.code' => $orderProductList[$j]['vendorItemCode']));
								if ($image) {
									foreach ($image->result_array() as $items) {
										if ($items["itemPhoto"] != "") {
											$orderProductList[$j]['itemImage'] = base_url() . 'partner/uploads/' . $clientOrderList[$i]['vendorCode'] . '/vendoritem/' . $items['itemPhoto'];
										} else {
											$orderProductList[$j]['itemImage'] == base_url() . 'partner/uploads/product.jpg';
										}
									}
								}
							}
							...
							$order['totalItems'] = sizeof($orderProductList);
							$order['orderedItems'] = $orderProductList;
							$dFormat = DateTime::createFromFormat('Y-m-d H:i:s', $clientOrderList[$i]['orderDate']);
							$oDt = $dFormat->format('d-m-Y H:i:s');
							$order["orderDate"] = $oDt;
						}
		...
		if ($orderVegiz && $orderVegiz->num_rows() > 0) {
			...
			if ($resultQuery) {
				...
				for ($i = 0; $i < sizeof($clientOrderList); $i++) {
					...
					$city = $this->db->select("latitude,longitude")->from("citymaster")->where("citymaster.code", $cityCode)->get()->row_array();
					if (!empty($city)) {
					...
					$clientOrderList[$i]['vendorName'] = $vName;
					$clientOrderList[$i]['vendorAddress'] = $vAddress;
					$clientOrderList[$i]['vendorContact'] = $vContact;
