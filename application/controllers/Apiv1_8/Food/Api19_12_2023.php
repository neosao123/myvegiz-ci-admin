...
class Api extends REST_Controller
{
		...
		if (isset($getData['vendorCode']) && $getData['vendorCode'] != "") {
			...
			if ($Records) {
				...
				foreach ($Records->result_array() as $ra) {
					...
					$orderColumns2 = array("vendoritemmaster.*,vendor.entityName,vendor.isServiceable as vendorIsServiceable,vendor.tagCode,tagmaster.tagTitle,tagmaster.tagColor");
					...
					if ($subCateRecords) {
						...
						foreach ($subCateRecords->result_array() as $subrow) {
							...
							$orderColumns4 = array("vendoritemmaster.*,vendor.entityName,vendor.isServiceable as vendorIsServiceable,vendor.tagCode,tagmaster.tagTitle,tagmaster.tagColor");
	...
	public function verifyPayment_post()
	{
		...
		if (isset($postData['clientCode']) && $postData['clientCode'] != "" && isset($postData['paymentOrderId']) && $postData['paymentOrderId'] != "" && isset($postData['paymentOrderToken']) && $postData['paymentOrderToken'] != "" && isset($postData['orderCode']) && $postData['orderCode'] != "") {
			...
			$clms = "vendorordermaster.paymentOrderId,vendorordermaster.paymentOrderToken,vendorordermaster.paymentStatus";
...
/*
	...
	public function sendCustomerNotification($clientCode, $dataNoti)
	{
		...
		$limit = 1;
		$checkdevices = $this->GlobalModel->selectQuery('clientmaster.firebaseId', 'clientmaster', array('clientmaster.clientCode' => $clientCode), $orderBy, $join, $joinType, $like, $limit);
		if ($checkdevices) {
			$DeviceIdsArr = array();
