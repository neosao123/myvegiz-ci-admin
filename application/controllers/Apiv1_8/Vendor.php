			...
			if ($result) {
				...
				if ($resultData) {
					...
					if ($resdata['entityImage'] != "") {
						$path = 'uploads/vendor/' . $resdata['code'] . '/' . $resdata['entityImage'];
						if (file_exists($path)) {
					...
					return $this->response(array("status" => "300", "message" => "Invalid mobile number or password."), 200);
				}
			} else {
				return $this->response(array("status" => "300", "message" => "Invalid mobile number or password."), 200);
			}
		...
		if (isset($getData['vendorCode']) && $getData['vendorCode'] != "") {
			...
			if ($Records) {
				foreach ($Records->result_array() as $ra) {
					...
					$like2 = array("vendoritemmaster.itemName" => $getData['search'] . '~both');
					...
					if ($subCateRecords) {
						...
						foreach ($subCateRecords->result_array() as $subrow) {
							...
							$like4 = array("vendoritemmaster.itemName" => $getData['search'] . '~both');
...
// 			$condition['deliveryBoyCode!='] = '';
			$condition=array("vendorCode"=>$resCode,"orderStatus"=>'PLC',"isActive"=>1);
			...
			$dataPlaceOrder = $this->GlobalModel->selectQuery('COUNT(id) as countPlaced', 'vendorordermaster', array('vendorordermaster.vendorCode' => $resCode,'vendorordermaster.isActive'=>1), array(), array(), array(), array(), '', '', array(), $extraCondition);
			if ($dataPlaceOrder) {
			...
			$condition1['isActive'] = '1';
			$condition1['isDelete'] = '0';
			...
			$condition4['isActive'] = '1';
			$condition4['isDelete'] = '0';
			...
			$condition5['isActive'] = '1';
			$condition5['isDelete'] = '0';
      ...
      //echo $this->db->last_query();  
			...
			$condition6['isActive'] = '1';
			$condition6['isDelete'] = '0';
			...
			if ($checkOrder->num_rows() > 0) {
				...
				switch ($postData['orderStatus']) {
					...
					case 'PRE':
						...
						$data['editDate'] = $timeStamp; 
						$data["orderAcceptedTime"]=date("Y-m-d H:i:s");
	...
	public function getBillingList_post()
	{
		...
		if (isset($postData['vendorCode']) && $postData['vendorCode'] != "") {
			...
			if (isset($postData['selectedDate']) && $postData['selectedDate'] != "") {
				$selectedDate = date('Y-m-d', strtotime(str_replace("/", "-", $postData['selectedDate'])));
			}
			...
			if($vendorComissionResult){
			  $commission = $vendorComissionResult->result_array()[0]["defaultVendorCommission"];
			}
			
			
			$this->db->select("vendorordermaster.code, vendorordermaster.grandTotal, vendorordermaster.discount, vendorordermaster.shippingCharges, vendorordermaster.tax, vendorordermaster.totalPackgingCharges,vendorordermaster.addDate,vendorordercommission.comissionAmount as adminComission,vendorordercommission.vendorAmount as payableAmount");
			$this->db->from("vendorordermaster");			
			$this->db->join('vendorordercommission', 'vendorordercommission.orderCode = vendorordermaster.code', 'inner');
			$this->db->where('vendorordermaster.vendorCode', $vendorCode);
			$this->db->where('vendorordermaster.isActive', 1);
			$this->db->where('vendorordermaster.orderStatus', 'DEL');
			if($selectedDate!=""){
			  $this->db->where("(Date(vendorordermaster.addDate))", $selectedDate);
			}
	...
	public function updatePreparingTime_post(){
		$postData = $this->post();
		$timeStamp = date("Y-m-d h:i:s");
		if (isset($postData['orderCode']) && $postData['orderCode'] != "" && isset($postData['preparingTime']) && $postData['preparingTime'] != "") {
			$data = array('preparingMinutes' => $postData['preparingTime'], 'editDate' => $timeStamp);  
