...
class Grocerycategory extends CI_Controller {
	var $session_key;
	...
	public function listRecords()
    {
    	...
    	$this->load->view('dashboard/grocerycategory/list',$data);
    ...
    {
    	$this->load->view('dashboard/header');
    	$this->load->view('dashboard/grocerycategory/add');
    	$this->load->view('dashboard/footer');
    ...
    public function edit()
    {
    	...
    	$this->load->view('dashboard/header');
    	$this->load->view('dashboard/grocerycategory/edit',$data);
    	$this->load->view('dashboard/footer');
			...
			{ 	
				...
				$actionHtml='<div class="btn-group">
								...
								<div class="dropdown-menu animated slideInUp" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 35px, 0px);">
									...
									<a class="dropdown-item" href="'.base_url().'index.php/Grocerycategory/edit/'.$row->code.'"><i class="ti-pencil-alt"></i> Edit</a>
    ...
    public function save()
    {
		...
		$text = $role." ".$userName.' added new grocerycategory "'.$categoryName.'" from '.$ip; 
		...
		$condition2 = array('categorySName'=>$categorySName,'mainCategoryCode'=>'MCAT_2');
        $result = $this->GlobalModel->checkDuplicateRecordNew($condition2,'categorymaster');
		
		if ($result==true) 
		{
			$data = array('error_message' => 'Duplicate Grocery Category short name');
			$this->load->view('dashboard/header');
			$this->load->view('dashboard/grocerycategory/add', $data);
			$this->load->view('dashboard/footer');
		}
		else
		{
		
		    $this->form_validation->set_rules('categoryName', 'Grocerycategory Name', 'required');
		    $this->form_validation->set_rules('categorySName', 'Grocerycategory Short Name', 'required');
			...
			{	 
				...
				$code = $this->GlobalModel->addWithoutYear($data, 'categorymaster', 'GCAT');
				// For Grocerycategory Image file upload 
				if($code!='false'){
				...
				if($code!='false')
				{
					...
					$response['message']="Grocery Category Successfully Added.";
					$this->GlobalModel->activityAdd($log_text,'activitymaster','ACT');
				}
				else
				{
					$response['status']=false;
					$response['message']="Failed To Add Grocery Category";
				}
				$this->session->set_flashdata('response', json_encode($response));		
				redirect(base_url() . 'index.php/Grocerycategory/listRecords', 'refresh');
			}   
		}
	...
	public function update()
	{
		...
		$ip=$_SERVER['REMOTE_ADDR'];
		$text = $role." ".$userName.' updated Grocerycategory "'.$categoryName.'" from '.$ip; 
		...
		$condition2 = array('categorySName'=>$categorySName,'mainCategoryCode'=>'MCAT_2','code!='=>$code);
        $result = $this->GlobalModel->checkDuplicateRecordNew($condition2,'categorymaster');
		
		if ($result==true) 
		{
			$data = array('error_message' => 'Duplicate Grocerycategory short name');
			$data['query'] = $this->GlobalModel->selectDataById($code,'categorymaster');
			$this->load->view('dashboard/header');
			$this->load->view('dashboard/grocerycategory/edit',$data);
			$this->load->view('dashboard/footer');
		...
		else
		{
			...
			{	 
				...
				if(trim($this->input->post("isActive"))!=1){
					...
					{
						$response['status']=true;
						$response['message']="Grocery Category Successfully Updated.";
						$this->GlobalModel->activityAdd($log_text,'activitymaster','ACT');
					}
					else
					{
						$response['status']=false;
						$response['message']="No change In Grocery Category";
					}
					$this->session->set_flashdata('response', json_encode($response)); 
					redirect(base_url() . 'index.php/Grocerycategory/listRecords', 'refresh');
			}
    ...
    public function delete()
    {
		...
		$text = $role." ".$userName.' deleted currency "'.$categoryName.'" from '.$ip; 
