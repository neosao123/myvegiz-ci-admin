...
class Groceryslider extends CI_Controller {
    var $session_key;
    ...
    public function listRecords() {
        ...
        $this->load->view('dashboard/groceryslider/list', $data);
    ...
    public function getSliderList() {
		...
		$condition = array('citymaster.isActive'=>1,'citymaster.code'=>$cityCode,'productslider.type'=>$type,'productslider.mainCategoryCode'=>'MCAT_2');
		...
		if($Records){ 
			foreach ($Records->result() as $row) {
				if($row->imagePath!=""){
					...
					$path = base_url().'assets/admin/assets/images/users/1.jpg';
				}
				...
				$actionHtml = '<div class="btn-group">
									...
									<div class="dropdown-menu animated slideInUp" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 35px, 0px);">
										...
										<a class="dropdown-item" href="' . base_url() . 'index.php/Groceryslider/edit/' . $row->code . '"><i class="ti-pencil-alt"></i> Edit</a>
    ...
    public function add() {
        ...
        $this->load->view('dashboard/header');
        $this->load->view('dashboard/groceryslider/add', $data);
        $this->load->view('dashboard/footer');
    }
    public function save() {
        ...
        $text = $role . " " . $userName . ' added new Grocery slider for citycode "' . $productCode . '" from ' . $ip;
        $log_text = array('code' => "demo", 'addID' => $addID, 'logText' => $text);
        $data = array('productCode' => $productCode,'type' => $this->input->post('type'), 'addID' => $addID, 'addIP' => $ip, 'isActive' => $this->input->post("isActive"),'mainCategoryCode'=>'MCAT_2');
        ...
        if ($code != 'false' && $filedoc!='false') {
            ...
            $response['message'] = "Grocery Slider Successfully Added.";
            $this->GlobalModel->activityAdd($log_text, 'activitymaster', 'ACT');
        } else {
            $response['status'] = false;
            $response['message'] = "Failed To Add Grocery Slider";
        }
        $this->session->set_flashdata('response', json_encode($response));
        redirect(base_url() . 'index.php/Groceryslider/listRecords', 'refresh');
    }
    ...
    public function edit() {
        ...
        $data['citymaster'] = $this->GlobalModel->selectData('citymaster');
        $this->load->view('dashboard/header');
        $this->load->view('dashboard/groceryslider/edit', $data);
        $this->load->view('dashboard/footer');
    }
    public function update() {
        ...
        $ip = $_SERVER['REMOTE_ADDR'];
        $text = $role . " " . $userName . ' updated Grocery slider for citycode "' . $productCode . '" from ' . $ip;
        $log_text = array('code' => "demo", 'addID' => $addID, 'logText' => $text);
        ...
        if ($result != 'false' || $filedoc!='false') {
            $response['status'] = true;
            $response['message'] = "Grocery Slider Successfully Updated.";
            $this->GlobalModel->activityAdd($log_text, 'activitymaster', 'ACT');
        } else {
            $response['status'] = false;
            $response['message'] = "No change In Grocery Slider" . $error;
        }
        $this->session->set_flashdata('response', json_encode($response));
        redirect(base_url() . 'index.php/Groceryslider/listRecords', 'refresh');
    }
    public function delete() {
        ...
        $text = $role . " " . $userName . ' deleted Grocery slider for citycode "' . $productName . '" from ' . $ip;
        $log_text = array('code' => "demo", 'addID' => $addID, 'logText' => $text);
        $this->GlobalModel->activityAdd($log_text, 'activitymaster', 'ACT');
    ...
    public function view() {
		...
		if($Records){
			...
			$text = $role . " " . $userName . ' viewed Grocery slider of code "' . $code . '" from ' . $ip;
			$log_text = array('code' => "demo", 'addID' => $addID, 'logText' => $text);
			$this->GlobalModel->activityAdd($log_text, 'activitymaster', 'ACT');
			// Activity Track Ends                
		...
		$this->load->view('dashboard/groceryslider/view',$data);
    }
