...
class Grocerysubcategory extends CI_Controller
{
	var $session_key;
	...
	public function listRecords()
	{
		...
		$this->load->view('dashboard/grocerysubcategory/list', $data);
			...
			foreach ($Records->result() as $row) {
				...
				$actionHtml = '<div class="btn-group">
								...
								<div class="dropdown-menu animated slideInUp" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 35px, 0px);">
									...
									<a class="dropdown-item" href="' . base_url() . 'Grocerysubcategory/edit/' . $row->code . '"><i class="ti-pencil-alt"></i> Edit</a>
	...
	public function save()
	{
		...
		$text = $role . " " . $userName . ' added new grocery subcategory "' . $subcategoryName . '" from ' . $ip;
		$log_text = array(
			'code' => "demo",
			'addID' => $addID,
			'logText' => $text
		...
		if ($result->num_rows() > 0) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$code = $this->GlobalModel->addWithoutYear($data, 'subcategorymaster', 'GSCAT');
				if ($code != 'false') {
				...
				redirect(base_url() . 'Grocerysubcategory/listRecords', 'refresh');
			}
		}
	}
	...
	public function update()
	{
		...
		$ip = $_SERVER['REMOTE_ADDR'];
		$text = $role . " " . $userName . ' updated grocery subcategory "' . $subcategoryName . '" from ' . $ip;
		$log_text = array(
			'code' => "demo",
			'addID' => $addID,
			'logText' => $text
	...
	public function delete()
	{
		...
		$text = $role . " " . $userName . ' deleted grocery subcategory "' . $subcategoryName . '" from ' . $ip;
		$log_text = array(
			'code' => "demo",
			'addID' => $addID,
			'logText' => $text
	...
	public function view()
	{
		...
		$tableName = "subcategorymaster";
		$orderColumns = array("subcategorymaster.*,categorymaster.categoryName");
		$condition = array('subcategorymaster.isActive' => 1, 'categorymaster.mainCategoryCode' => 'MCAT_2');
		$orderBy = array('subcategorymaster' . '.id' => 'DESC');
		$joinType = array('categorymaster' => 'inner');
		$join = array('categorymaster' => 'categorymaster.code=subcategorymaster.categoryCode');
		$groupByColumn = array();
		...
		foreach ($Records->result() as $row) {
			...
			$text = $role . " " . $userName . ' viewed grocery subcategory "' . $row->subcategoryName . '" from ' . $ip;
			$log_text = array(
				'code' => "demo",
				'addID' => $addID,
				'logText' => $text
