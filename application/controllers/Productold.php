...
class Product extends CI_Controller
{
	...
	public function save()
	{
		...
		if ($result == true) {
			...
			$this->form_validation->set_rules('productSubCategory', 'Product Sub Category Name', 'required');
	...
	public function view()
	{
		...
		foreach ($Records->result() as $row) {
			...
			$condition1 = array("productratelineentries.productCode" => $code);
			$orderBy1 = array();
			...
			foreach($res->result() as $price)
			{
				...
				$priceData.='<div class="col-md-4"><label>' . $price->cityName . '</label></div><div class="col-md-4"><label>' . $price->sellingPrice . '</label></div><div class="col-md-4"><label>' . $sta . '</label></div>';
				$j++;
			}
			...
			$modelHtml .= '<div class="form-row">
						...
						<label> <b>Product Selling Price :</b> </label>
						<div class="form-row">
						' . $priceData . '
						</div>
