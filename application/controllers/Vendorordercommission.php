	...
	{   
		...
		if ($Records) {
			foreach ($Records->result() as $row) {
			   ...
			   $paidCommission=0;
			   $remCommission=0;
			   $query=$this->db->query("select ifNull(sum(vendorordercommission.vendorAmount),0) as paidcommission from vendorordercommission where deliveryBoyCode='".$row->deliveryBoyCode."' and (addDate between '".$startDate." 00:00:00' AND '".$endDate." 23:59:59') and isPaid ='1'");
			   if($query){
			   ...
			   $remCommission=$row->vendorAmount-$paidCommission;      
                $actionHtml = '
				<div class="btn-group">
