...
class UserCancelOrder extends CI_Controller 
{
	...
	{ 
        ...
        $requiredColumns = array(array('code', 'clientCode','orderStatus', 'areaCode', 'address', 'phone', 'totalPrice', 'isActive', 'addDate'), array('name'));
		  ...
		  if ($orderStatus !== '' && $placeList == 0)
            {
                ...
                /*added by anirudha on 15-3-21*/
				$extraCondition = "ordermaster.orderStatus ='CAN' OR ordermaster.orderStatus='RJT'";  
				/*added by anirudha on 15-3-21*/
			}
        ...
        $orderColumnsArray = array("ordermaster.*,ordermaster.code as orderCode,ordermaster.addDate as orderaddDate,ordermaster.editID as orderEditID,clientmaster.*,clientprofile.pincode,customaddressmaster.*,citymaster.cityName"); 
        ...
        $groupByColumn = array();
		//$GROUPBY=array('ordermaster.code');
		...
		if ($Records)
        {
            ...
            foreach ($Records->result() as $row)
            {
                ...
                switch ($orderStatus)
                {
                    ...
                    case "RJT":
						$orderStatus = "Reject";
						$orderDate = date('d-m-Y h:i:s', strtotime($row->$rejectedTime));
				    break;
                }
