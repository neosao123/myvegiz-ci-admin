...
class Firestore
{
     ...
     */
    public function update_order_status(string $orderCode, string $status, $orderType = "")
    {
	...
	public function add_deliveryboy(array $data){
		$project_id = FIRESTORE_PROJECT_ID;
        $firestore_key = FIREBASE_KEY;
        $firestore_data  = [
            'bearing' => ["stringValue" => $data["bearing"]??""],
			'id' => ["stringValue" => $data["id"]],
			'latitude' => ["stringValue" => $data["latitude"]??""],
            'longitude' => ["stringValue" => $data["longitude"]??""]
        ];
		$fireData = ["fields" => (object)$firestore_data];
        $json = json_encode($fireData);
		$url = "https://firestore.googleapis.com/v1beta1/projects/$project_id/databases/(default)/documents/DeliveryMan/" . $data["id"];
		$curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_CUSTOMREQUEST => 'POST',
