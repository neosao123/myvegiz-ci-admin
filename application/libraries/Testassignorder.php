    ...
    function allocate_delivery_boy_to_order(string $orderCode)  {
        ...
        if($order_result) {            
            ...
            if($restaurant_result) {
				...
				echo 'Vendor : '.$restaurant_lat.'#'.$restaurant_long;
                ...
                if(($restaurant_lat!="" && $restaurant_long!=null) && ($restaurant_long!="" && $restaurant_long!=null)) {
					...
					echo "<br>DB Query: SELECT usermaster.code, ( 3959 * acos( cos( radians(16.69513131426673) ) *
cos( radians( usermaster.latitude ) ) * cos( radians( usermaster.longitude ) - radians(74.60179988456422) ) + sin(
radians(16.69513131426673) ) * sin(radians(usermaster.latitude)) ) ) AS distance FROM usermaster INNER JOIN
deliveryBoyActiveOrder ON deliveryBoyActiveOrder.deliveryBoyCode = usermaster.code WHERE usermaster.role='DLB' AND
usermaster.isActive='1' AND usermaster.deliveryType='food' AND deliveryBoyActiveOrder.loginStatus='1' AND
deliveryBoyActiveOrder.orderCount='0' AND deliveryBoyActiveOrder.isActive='1' HAVING distance < 5 ORDER BY distance ";
					
                    if(!empty($query_result)) {
                        $row_count = count($query_result);  
						echo '<br>Total DB : '.$row_count;
