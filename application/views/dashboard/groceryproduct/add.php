		...
		<div class="card">
			<div class="card-body">
				...
				<form class="needs-validation" method="post" id="myForm" action="<?php echo base_url() . 'index.php/Groceryproduct/save'; ?>" enctype="multipart/form-data" novalidate>
					...
					<div class="form-group prrow removeclassP0" id="row0">
						...
						<div class="form-row mb-3">
							<div class="col-md-3 mb-2">
								<input type="number" step="0.01" class="form-control regularPrice" id="regularPrice0" name="regularPrice[]" required placeholder="Regular Price (0.00)" onchange="checkRegularPrice(0)">
							</div>
							<div class="col-md-4">
								<input type="number" step="0.01" class="form-control sellingPrice" id="sellingPrice0" name="sellingPrice[]" required placeholder="Selling Price (0.00)" onchange="checkSellingPrice(0)">
							</div>
							<div class="col-md-3">
								...
								?>
									<div class="col-sm-3 mb-3">
										<div class="custom-control custom-radio custom-control-inline">
											...
											<label class="custom-control-label" style="color:<?= $tag['tagColor'] ?>" for="tagSection<?= $tag['code'] ?>"><b><?= ucwords($tag['tagTitle']) ?></b></label>
										</div>
										<!--<div class="box" style="background-color:<?= ucwords($tag['tagColor']) ?>"></div>-->
									</div>
								<?php } ?>
							</div>
						</div>
...
<script>
	...
	function price_fields(count, flag) {
		...
		elements += '<div class="col-md-4 mb-2"><input type="number" step="0.01" class="form-control sellingPrice" id="sellingPrice' + room + '" name="sellingPrice[]" required placeholder="Selling Price (0.00)"  onchange="checkSellingPrice(' + room + ')"></div>'
	...
	$("body").delegate(".remove_price_fields", "click", function() {
		...
		if (arr.length > 1) {
			...
			swal("Product", "Product should have at least one variant.", "error");
		}
	});
