<div class="page-wrapper">
				...
				<div class="d-flex align-items-center">
					<nav aria-label="breadcrumb">
						<ol class="breadcrumb">
							...
							<li class="breadcrumb-item active" aria-current="page">Change In Grocery Product</li>
						</ol>
					</nav>
				</div>
			</div>
	...
	<div class="container-fluid col-md-8">
		<div class="card">
			<div class="card-body">
				<h4 class="card-title"> Change In Grocery Product</h4>
				<hr />

				<form class="needs-validation" method="post" id="service" action="<?php echo base_url() . 'index.php/Groceryproduct/update'; ?>" enctype="multipart/form-data" novalidate>
					...
					?>
						...
						<div class="form-row">
							...
							<div class="col-md-6 mb-3">
								...
								<div class="controls">
									...
									<select class="custom-select form-control" id="productSubCategory" name="productSubCategory">
						...
						<div class="form-row">
							...
							<div class="col-md-4 mb-3">
								...
								<input type="text" class="form-control" id="hsnCode" name="hsnCode" value="<?= $row->hsnCode; ?>">
							</div>
							<div class="col-md-4 mb-3">
						...
						<div class="form-row">
							<div id="benefits_fields" class="col-md-12 m-t-20">
								...
								foreach ($benefits->result() as $line) { ?>
									...
									<div class="form-group removeclass<?php echo $j ?>" style="margin-bottom:0.5rem;">
										<div class="form-row">
											<div class="col-md-10">
						...
						<div id="price_fields">
							...
							?>
									<div class="prrow removeclassP<?php echo $j ?>" id="row<?php echo $j ?>">
										<div class="form-row mb-1">
											<input type="hidden" name="lineCodes[]" id="lineCodes<?php echo $j ?>" value="<?= $line->code ?>">
											<div class="col-md-4">
										...
										<div class="form-row mb-1">
											<div class="col-md-3">
												<input type="number" step="0.01" id="regularPrice<?= $j ?>" required name="regularPrice[]" value="<?= $line->regularPrice ?>" class="form-control regularPrice" placeholder="Regular Price (0.00)" onchange="checkRegularPrice(<?= $j ?>)">
											</div>
											<div class="col-md-3">
												<input type="number" step="0.01" id="sellingPrice<?= $j ?>" required name="sellingPrice[]" value="<?= $line->sellingPrice ?>" class="form-control sellingPrice" placeholder="Selling Price (0.00)" onchange="checkSellingPrice(<?= $j ?>)">
											</div>
											<div class="col-md-3">
					...
					<div class="text-xs-right">
						...
						<a href="<?php echo base_url() . 'index.php/Groceryproduct/listRecords'; ?>">
...
<script>
	...
	function deleteButton(value) {
		...
		$.ajax({
			url: base_path + 'Groceryproduct/deleteImage',
			method: "POST",
			data: {
				'value': value
			},
	...
	function get_sub_units(main_unit) {
		if (main_unit != "") {
			$.ajax({
				type: "get",
				url: base_path + 'Groceryproduct/getSubUnits',
				data: {
					unit: main_unit
				},
				success: function(response) {
					if (response) $("#subunitlist").html(response);
					$("#subUnit0").html(response);
				},
	...
	function checkSellingPrice(id) {
		var sp = $("#sellingPrice" + id).val().trim();
		var rp = $('#regularPrice' + id).val().trim();
		if (sp != "" && rp != "") {
			if (isNaN(sp)) {
				swal("Selling Price", "Selling price must be valid amount", "error");
				$("#sellingPrice" + id).focus();
				return false;
			} else if (Number(sp) <= 0) {
	...
	function checkDuplicateProduct(id) {
		...
		for (row_id = room; row_id > 0; row_id--) {
			...
			if (id != row_id) {
				if ((row_quantity == quantity && row_subUnit == subUnit && row_cityCode == cityCode)) {
	...
	function price_fields(count, flag) {
		...
		if (unit != "") {
			...
			var divtest = document.createElement("div");
			divtest.setAttribute("class", "form-group prrow removeclassP" + room);
			divtest.setAttribute("id", "row" + room);
			...
			elements += '<input type="hidden" name="lineCodes[]" id="lineCodes' + room + '" value="-">';
			...
			elements += '<div class="col-md-3 mb-2"><input type="number" step="0.01" class="form-control regularPrice" id="regularPrice' + room + '" name="regularPrice[]" required  placeholder="Regular Price (0.00)" onchange="checkRegularPrice(' + room + ')"></div>'
			elements += '<div class="col-md-3 mb-2"><input type="number" step="0.01" class="form-control sellingPrice" id="sellingPrice' + room + '" name="sellingPrice[]" required placeholder="Selling Price (0.00)" onchange="checkSellingPrice(' + room + ')"></div>'
	...
	function remove_benefits_fields(rid, flag, code) {
		if (flag == 'edit') {
			swal({
				...
				if (e) {
					$.ajax({
						url: base_path + "Groceryproduct/deleteLineRecord",
						type: 'POST',
						data: {
							'code': code
						},
	...
	function remove_price_fields(rid, flag, code) {
		if (flag == 'edit') {
			swal({
				...
				if (e) {
					$.ajax({
						url: base_path + "Groceryproduct/deletePriceRecord",
						type: 'POST',
						data: {
							'code': code
						},
			...
			if (html.length == 0) {
				...
				swal({
					...
					if (e) {
						$.ajax({
							type: "get",
							url: base_path + 'Groceryproduct/delete_all_rate_entries',
							data: {
