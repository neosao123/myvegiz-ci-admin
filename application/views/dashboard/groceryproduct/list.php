		...
		<div class="card">
        <div class="card-body">
         <h3 class="card-title"> Filter Grocery Product :</h3>
	...
	echo '<option value="' . $cat->categorySName . '">' . $cat->categoryName . '</option>';
			   ...
			   <div class="col-sm-4">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title">Grocery Product List</h4>
                                
                                <div class="table-responsive">
                                    <table id="datatableProductG" class="table table-striped table-bordered">
                                        <thead>
                                            <tr>
                                                <th>Sr.No.</th>
                                                <th>Code</th>
		   ...
		   $( document ).ready(function() {
			...
			$('#productDateEnd').datepicker({
			   ...
			   var keyProductCode="",keyProductDateStart="",keyProductDateEnd="",keyStorageCode="",keyCategoryCode="";
			getDataTable('','','','','');
			function clearGlobalVariables()
			{
				...
				$('#datatableProductG').DataTable().state.clear();
			 ...
			 function getDataTable(p_keyProductCode,p_keyStorageCode,p_keyCategoryCode,p_keyProductDateStart,p_keyProductDateEnd)
			{
				...
				if ($.fn.DataTable.isDataTable("#datatableProductG")) {
					$('#datatableProductG').DataTable().clear().destroy();
				}
				//alert(p_keyCategoryCode);
				//alert(p_keyProductDateEnd);
			   var dataTable = $('#datatableProductG').DataTable({  
					stateSave: true,
					 "processing":true,  
				   "serverSide":true,  
				   "order":[],
				   "searching": true,
				   "ajax":{  
						url: base_path+"Groceryproduct/getGroceryProductList", 
							data:{
			 ...
			 $('.mywarning').on("click", function() {
				var code=$(this).data('seq');
					swal({
						...
						{
							$.ajax({
								url: base_path+"Groceryproduct/delete",
								type: 'POST',
								data:{
								  'code':code
								},
...
$( document ).ready(function() {
    ...
    if(data!='')
    {
		...
		if(obj.status)
		{
			toastr.success(obj.message, 'Product', { "progressBar": true });
		}
		else
		{
			toastr.error(obj.message, 'Product', { "progressBar": true });
		}
    }
	//end show alerts
});
