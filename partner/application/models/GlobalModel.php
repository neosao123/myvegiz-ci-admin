...
class GlobalModel extends CI_Model
{
    ...
    public function checkDuplicateRecordNew($tablename,$condition=array())
    {
        $this->db->where($condition);
	...
	{
		$query = $this->db->query("SELECT * FROM `" . $tblname . "` WHERE " . $field . "='" . $value . "' AND vendorCode != '" . $code . "' AND (`isDelete` IS NULL OR `isDelete`='0')");
		$count_row = $query->num_rows();
		if ($count_row > 0) {
			$stmt = true;
		} else {
