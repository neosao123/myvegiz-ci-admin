                                    ...
                                    <?php
                                        if($categories)
                                        {
                                            foreach($categories->result_array() as $c)
                                            {
                                                ...
                                                echo '<a class="nav-link" id="'.$c['code'].'-tab" data-toggle="pill" href="#'.$c['code'].'" role="tab" aria-controls="'.$c['code'].'" aria-selected="true">'.$c['categoryTitle'].' - '.$categoryType.' <span class="d-none mr-1 ml-2 btn btn-warning btn-xs deleteCustomizedCategory" id="deltab_'.$c['code'].'"><i class="fa fa-trash"></i></span><span class="d-none mr-1 btn btn-danger btn-xs editCustomizedCategory" id="edttab_'.$c['code'].'"><i class="ti-pencil-alt"></i></span></a>';;
                                            }
                                        }
                                    ?>
                                </div>
...
<script>
	...
	$("body").on('click',".lineDelete",function(e){
		var urldelete = base_path+'index.php/Vendoritem/deleteAddonLine'; 
	...
	$("body").on('click',".addSubCategory",function(e){
		...
		var url = base_path+'index.php/Vendoritem/addAddonLine'; 
		var isCateEnabled  = 0;
				...
				if(subTitle.length>2){
					$.ajax({
						...
						{  
							...
							{
								toastr.success(res.message, 'Vendor Item', { "progressBar": true });
							}
						} 
					});
				}else{ 
	...
	$("body").on('click',"#addCustomizedCategory",function(e){ 
		...
		var url = base_path+'index.php/Vendoritem/addAddonCategory';
		if(customizedCategoryCode!="" && customizedCategoryCode!=undefined) url = base_path+'index.php/Vendoritem/updateAddonCategory'
		var isCateEnabled  = 0;
				...
				success:function(response)
				{
					...
					if(res.status=='true')
					{
						if(customizedCategoryCode!="" && customizedCategoryCode!=undefined){
							...
							$('#'+customizedCategoryCode+'-tab').html(categoryTitle+' - '+categoryTypeTitle+' <span class="mr-1 btn btn-warning btn-xs deleteCustomizedCategory" id="deltab_'+customizedCategoryCode+'"><i class="fa fa-trash"></i></span><span class="mr-1 btn btn-danger btn-xs editCustomizedCategory" id="edttab_'+customizedCategoryCode+'"><i class="ti-pencil-alt"></i></span>');
							...
							var html = '<a class="nav-link active" id="'+res.code+'-tab" data-toggle="pill" href="#'+res.code+'" role="tab" aria-controls="'+res.code+'" aria-selected="true">'+categoryTitle+' - '+categoryTypeTitle +'</a>';
	...
	$("body").on('click',".deleteCustomizedCategory",function(e){
		...
		$.ajax({
			url: base_path+'index.php/Vendoritem/deleteAddonCategory',
			data:{'code':code},
			type:'post',
			success:function(response)
			{   
	...
	$("body").on('click',".editCustomizedCategory",function(e){
		...
		code = code.substring(7);
		$.ajax({
			url: base_path+'index.php/Vendoritem/getAddonCategoryData',
			data:{'code':code},
			type:'post',
			success:function(response)
			{   
