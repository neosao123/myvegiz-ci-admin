...
<div class="page-wrapper">
    ...
    <div class="container-fluid col-md-12">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <form class="needs-validation" method="post" id="order" action="<?php echo base_url().'index.php/Pendingorders/confirm';?>" novalidate>
					...
					<div class="form-row">
						...
						<div class="col-md-3 mb-3">
							<label for="coupanCode"> Coupan Code  : </label>
					...
					<hr>
							  <div class="table-responsive">
								...
								<div class="form-row ">
										...
										<?php if(!empty($row->coupanCode)){?>
    									<div class="col-md-3 mt-3 text-left">
    										<label><h4>Discount:</h4></label><label><h4><i>&nbsp;<?= $row->discount ?> (<?= $row->coupanCode ?>)</i></h4></label>
    									</div>
										<?php }?>
    									<div class="col-md-3 mt-3 text-left">
    										<label><h4>Tax:</h4></label><label><h4><i>&nbsp;<?= $row->tax ?></i></h4></label>
    									</div>
										<?php if(!empty($row->totalPackgingCharges) || $row->totalPackgingCharges!="0.00" ){?>
										<div class="col-md-3 mt-3 ">
											<label>
												<h4 class="">Packaging Charges:</h4>
											</label>
											<label>
												<h4 class=""><i>&nbsp;<?= $row->totalPackgingCharges ?></i></h4>
											</label>
										</div>
										<?php }?>
    									<div class="col-md-3 mt-3 text-left">
	...
	<script>
	   ...
	   var dataTable = $('#datatableOrderDetailsVendor').DataTable({  
		   ...
		   "searching": false,
           "ajax":{  
                url: base_path+"index.php/Pendingorders/getOrderDetails",  
                type:"GET" , 
		...
		$('#discard').click(function() {
			...
			swal({
				...
				{
					$.ajax({
						url: base_path+"index.php/Pendingorders/reject",
						type: 'POST',
						data:{'code':orderCode},
						success: function(data) {
							debugger;
							if(data!='false')
							{
								...
								function(isConfirm){
									if (isConfirm) {
										if(placeFlag==1){
											window.location.href = "<?= base_url().'index.php/Pendingorders'?>";
										}else{
											window.location.href = "<?= base_url().'index.php/Pendingorders'?>";
										}
									}
